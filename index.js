...
async function run() {
  try {
    ...
    const assetRequestCollection = dataBase.collection(
      "assetRequestCollection",
    );
    const packageCollection = dataBase.collection("packageCollection");
    const employeeRequestsCollection = dataBase.collection(
      "employeeRequestsCollection",
    );
    ...
    app.get(
      ...
      async (req, res) => {
        ...
        const assets = await assetCollection.find(query2).toArray();
    ...
    app.put("/asset-approval/:id", verifyToken, verifyHR, async (req, res) => {
      ...
      const assetCountUpdate = await assetCollection.updateOne(productQuery, {
    ...
    app.put(
      ...
      async (req, res) => {
        ...
        const result = await employeeRequestsCollection.updateMany(
          query,
          LatestUpdate,
        );
        res.send(result);
